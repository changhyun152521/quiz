...
router.get('/assignment/:assignmentId/course/:courseId', authenticate, authorize(['강사']), getAnswersByAssignmentAndCourse);

// GET /api/answers/assignment/:assignmentId - 특정 과제의 모든 정답 조회 (강좌 무관)
router.get('/assignment/:assignmentId', authenticate, authorize(['강사']), async (req, res) => {
  try {
...
router.get('/', authenticate, authorize(['강사']), getAllAnswers);

// GET /api/answers/:id - 특정 정답 조회
router.get('/:id', authenticate, authorize(['강사']), getAnswerById);

// POST /api/answers - 새 정답 생성 (또는 여러 정답 일괄 생성)
router.post('/', authenticate, authorize(['강사']), createAnswer);

// PUT /api/answers/:id - 정답 수정
router.put('/:id', authenticate, authorize(['강사']), updateAnswer);

// DELETE /api/answers/assignment/:assignmentId/course/:courseId - 특정 과제와 강좌의 모든 정답 삭제
router.delete('/assignment/:assignmentId/course/:courseId', authenticate, authorize(['강사']), deleteAnswersByAssignmentAndCourse);

// DELETE /api/answers/:id - 정답 삭제
router.delete('/:id', authenticate, authorize(['강사']), deleteAnswer);
